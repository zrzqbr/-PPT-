...
# -*- coding: utf-8 -*-
"""
apply_template.py — 腾讯云架构师长沙同盟 PPT 模板适配器 v8（统一入口）
...
=== PPTX 迁移模式 ===
六项品牌适配：
  1) 替换背景图（按页面类型：封面/扉页/结尾页用 bg-cover；内容页用 bg-content）
  ...
  5) 文字色对比度修复（P0-A）：
     ...
     - 浅色背景+浅色文字 → 自动翻转为深灰 #222222
  6) 元素溢出检测（P1-B）：
     - 检测内容是否溢出页面边界或侵入 Logo 安全区（仅报告不修改）
...
# ---------- 品牌安全色板（完整）----------
# 格式：(R, G, B, 描述)
# 注意：apply_template.py 的色彩判断（_is_forbidden_color + find_nearest_brand_color）
# 与 brand_palette.py 的 validate_color()（HSV 色相区间）是两套独立规则。
# 本脚本以 CIEDE2000 ΔE 球形半径为准，brand_palette.py 仅供 AI prompt 输出使用。
BRAND_PALETTE = [
    # §4.1 主色系（9色）
    (0xD8, 0x0C, 0x01, "品牌红（深）"),
    (0xFF, 0x00, 0x00, "品牌红（亮）"),
    (0x00, 0x00, 0x00, "经典黑"),
    (0x22, 0x22, 0x22, "深灰文字"),
    (0x66, 0x66, 0x66, "中灰"),
    (0x88, 0x88, 0x88, "浅灰"),
...
# ---------- 禁用色黑名单（蓝紫/青/鲜绿等）----------
# ⚠️ CIEDE2000 半径校准说明（v5 精确校准，反复实测）：
#
#   关键约束（品牌安全色最近距离 → 半径上限）：
#   商务蓝 #1D6FA9：
#     - 距 蓝紫#4B62E1      = 12.5  → 蓝紫半径 ≤ 11
#     - 距 亮蓝#004DE9      = 14.2  → 亮蓝半径 ≤ 13
#     - 距 亮蓝2#46B4E9     = 23.6  → 亮蓝2半径 ≤ 22
#     - 距 天蓝#0078D4      =  5.7  → 天蓝只能精准点覆盖（radius=4）
#     - 距 青绿#008080      = 21.9  → 青绿半径 ≤ 20
#   #08194B 深蓝已移入禁用色（见下方 FORBIDDEN_COLOR_RULES）
#   WarmTone 暖杏色 #D4A574：
#     - 距 荧光橙#FF7F00    = 15.9  → 荧光橙半径 ≤ 14
#     - 距 橙黄#FFC000      = 17.3  → 橙黄半径 ≤ 16
#   浅金橙 #E8B84A：
#     - 距 橙黄#FFC000      =  6.2  → 橙黄只能精准点覆盖（radius=5）
#   提示金 #F19D19：
#     - 距 荧光橙#FF7F00    = 11.4  → 荧光橙半径 ≤ 10
#     - 距 橙黄#FFC000      = 11.3  → 橙黄半径 ≤ 10
#   玫瑰木/浅赭色：
#     - 距 荧光黄#FFFF00    = 30~35 → 荧光黄半径 ≤ 29
#
#   ℹ️ 半径偏小导致部分禁用色有漏网，但误禁品牌色危害更大。
#      PPT 中蓝紫/荧光通常 de<5 到禁用中心，精准半径足够拦截。
FORBIDDEN_COLOR_RULES = [
    # 深蓝（#08194B 原品牌色，现禁用作内容色块）
    {"name": "深蓝",  "center": (0x08, 0x19, 0x4B), "radius": 10},

    # 蓝紫系
    {"name": "蓝紫",  "center": (0x4B, 0x62, 0xE1), "radius": 11},   # 商务蓝距此 12.5
    {"name": "蓝紫2", "center": (0x7B, 0x61, 0xFF), "radius": 19},   # 商务蓝距此 20.0
    {"name": "蓝紫3", "center": (0x9B, 0x59, 0xB6), "radius": 25},   # 商务蓝距此 29.5，中灰距此 26.1

    # 亮蓝/鲜蓝
    {"name": "亮蓝",  "center": (0x00, 0x4D, 0xE9), "radius": 13},   # 商务蓝距此 14.2
    {"name": "亮蓝2", "center": (0x46, 0xB4, 0xE9), "radius": 22},   # 商务蓝距此 23.6
    {"name": "天蓝",  "center": (0x00, 0x78, 0xD4), "radius": 4},    # 商务蓝距此 5.7（精准点）

    # 青色/Teal
    # 高级灰绿 #4A5D52 距青绿 de=17.3 → 青绿半径取 16
    {"name": "青色",  "center": (0x00, 0xCE, 0xD1), "radius": 25},
    {"name": "青绿",  "center": (0x00, 0x80, 0x80), "radius": 16},   # 高级灰绿距此 17.3

    # 鲜亮绿色
    {"name": "鲜绿",  "center": (0x00, 0xFF, 0x00), "radius": 30},
    {"name": "草绿",  "center": (0x1D, 0x9A, 0x78), "radius": 20},
    {"name": "翠绿",  "center": (0x2E, 0xCC, 0x71), "radius": 20},

    # 粉色/玫红
    {"name": "玫红",  "center": (0xFF, 0x14, 0x93), "radius": 20},   # 珊瑚粉距此 28.0
    {"name": "粉色",  "center": (0xFF, 0x69, 0xB4), "radius": 18},   # 珊瑚粉距此 23.7

    # 荧光（收紧到不覆盖暖色调）
    # 暖淡黄 #FAD16A 距荧光黄 de=16.4，浅金橙 #E8B84A 距荧光黄 de=20.6
    # 提示金 #F19D19 距荧光黄 de=30.6 → 半径取 15
    {"name": "荧光黄", "center": (0xFF, 0xFF, 0x00), "radius": 15},  # 暖淡黄距此 16.4
    {"name": "荧光橙", "center": (0xFF, 0x7F, 0x00), "radius": 10},  # 提示金距此 11.4

    # 补充荧光色
    # 米白底 #FAF6EE 距荧光绿2 de=23.6 → 半径取 20
    {"name": "荧光青绿", "center": (0x5F, 0xFF, 0xD2), "radius": 20},
    {"name": "荧光绿2",  "center": (0xAE, 0xFF, 0xAD), "radius": 20},  # 米白底距此 23.6
    {"name": "荧光水",   "center": (0x19, 0xFF, 0xF5), "radius": 20},
    {"name": "亮天蓝",   "center": (0x39, 0xBE, 0xED), "radius": 15},

    # 橙黄（#FFC000 主题色）
    # 浅金橙 #E8B84A 距此 6.2，提示金距此 11.3 → 半径只能精准覆盖
    {"name": "橙黄",     "center": (0xFF, 0xC0, 0x00), "radius": 5},

    # 绿色系
    {"name": "亮绿2",    "center": (0x00, 0xB0, 0x50), "radius": 20},

    # 深蓝紫（主题 accent6 常见 #2D2D89）
    {"name": "深蓝紫",   "center": (0x2D, 0x2D, 0x89), "radius": 10},  # 沉稳深蓝距此 11.3
]
...
def find_nearest_warm_color(r: int, g: int, b: int):
    """在暖色品牌色中找最近的颜色，用于内容色块优先映射。

    避免 CIEDE2000 机械匹配导致所有蓝色元素都映射到 #1D6FA9 商务蓝，
    而品牌规范要求内容色块优先使用暖色系（暖杏色、珊瑚粉、浅金橙等）。
...
def _fix_run_scheme_white(run, dry_run: bool = False) -> bool:
    ...
    如果检测到白色 scheme 引用，将其替换为 srgbClr #222222。
        ...
        try:
            ...
            if orig:
                ...
                if nearest:
                    ...
                    # 商务蓝 #1D6FA9 仅限数据系列/流程图，内容色块优先映射到暖色系
                    if (nr, ng, nb) == (0x1D, 0x6F, 0xA9):
                        warm = find_nearest_warm_color(*orig)
                        if warm:
                            nr, ng, nb, desc, de = warm
                    if (nr, ng, nb) != orig:
...
def get_assets(city: str = "changsha"):
    """返回背景与 Logo 路径。"""
    ...
    for p in (bg_cover, bg_content, logo_main):
        if not p.exists():
            raise FileNotFoundError(f"缺少模板资源：{p}")

    logo_corner = ASSETS / "logos" / "logo-corner.png"

    return {
        "bg_cover": str(bg_cover),
        "bg_content": str(bg_content),
        "logo_main": str(logo_main),
        "logo_corner": str(logo_corner) if logo_corner.exists() else None,
        "city": city,
    }
...
def check_element_overflow(slide, slide_w: int, slide_h: int,
                            page_type: str) -> list:
    """[P1-B] 检测内容元素是否溢出页面边界或侵入 Logo 安全区。
    ...
    # Logo 安全区定义（EMU）——根据 page_type
    # 封面/扉页/结尾：左上角 Logo 区域 (x0~x0+w, y0~y0+h)
    # 内容页：右上角 Logo 区域
    TEMPLATE_W_INCH = 20.02
    scale = slide_w / Inches(TEMPLATE_W_INCH)
    logo_w = int(Inches(4.88) * scale)
    logo_h = int(Inches(0.53) * scale)

    if page_type in ("cover", "section", "end"):
        logo_left = int(Inches(0.65) * scale)
        logo_top = int(Inches(0.61) * scale)
    else:
        logo_left = int(Inches(14.60) * scale)
        logo_top = int(Inches(0.49) * scale)

    logo_right = logo_left + logo_w
        ...
        try:
            ...
            if shp.has_text_frame and shp.text_frame.text.strip():
                ...
                if (shp.left < logo_right and shp_right > logo_left and
                    ...
                    warnings.append(f"形状 '{name}' 侵入 Logo 安全区")
        except Exception:
            pass
...
def add_logo(slide, page_type: str, logo_main: str, logo_corner: str,
    ...
    """根据页面类型在正确位置添加 Logo（P0-2 修复：插入前去重检测；v6 修复：等比缩放）。

    模板规则（来源：模板 Layout 实测，基准尺寸 20.02"x11.26"）：
    - 封面页(cover)/章节扉页(section)/结尾页(ending)：
        logo-main.png → 左上角 (基准 x=0.65", y=0.61", w=4.88", h=0.53")
    - 内容页(content)：
        logo-main.png → 右上角 (基准 x=14.60", y=0.49", w=4.88", h=0.53")

    注：logo-corner.png（左上角小图标）已移除，不再插入。
    注：实际位置按输入 PPT 实际尺寸等比缩放（基准 20.02"x11.26"）。

    若当前页已存在相同位置/尺寸的图片，则跳过插入，返回 False。
    """
    added = False

    # 基准模板尺寸（英寸）
    TEMPLATE_W_INCH = 20.02
    TEMPLATE_H_INCH = 11.26

    # 等比缩放因子：输入 PPT 宽度 / 模板宽度（EMU/EMU = 无单位比例）
    # 输入 PPT 与模板为同比例 16:9，宽高缩放因子应相同，用宽度计算即可
    scale = slide_w / Inches(TEMPLATE_W_INCH)

    # Logo 基准尺寸（英寸）→ 实际尺寸（EMU）
    logo_base_w = 4.88
    logo_base_h = 0.53
    main_w = int(Inches(logo_base_w) * scale)
    main_h = int(Inches(logo_base_h) * scale)

    if page_type in ("cover", "section", "end"):
        # 左上角
        main_left = int(Inches(0.65) * scale)
        main_top = int(Inches(0.61) * scale)

        if not _logo_already_exists(slide, main_left, main_top, main_w, main_h):
            slide.shapes.add_picture(logo_main, main_left, main_top,
                                     width=main_w, height=main_h)
            added = True
    else:
        # 右上角（距左边 14.60" × 缩放）
        main_left = int(Inches(14.60) * scale)
        main_top = int(Inches(0.49) * scale)
...
def _get_background_luminance(slide) -> float:
    ...
    try:
        ...
        pass
    # 默认：模板使用米白底 #FAF6EE（亮度 ≈ 0.92）
    return _relative_luminance(0xFA, 0xF6, 0xEE)
...
def fix_text_contrast(slide, page_type: str = "content", dry_run: bool = False) -> int:
    ...
    规则（基于模板为浅色背景的前提）：
    - 所有页面（bg_lum > 0.4）：
        - 封面/章节/结尾页（bg-cover 更浅）：文字亮度 > 0.40 → 改为 #222222
        - 内容页：文字亮度 > 0.50 且对比度 < 3:1 → 改为 #222222
            ...
            for run in para.runs:
                ...
                if orig is None:
                    if effective_bg_lum >= 0.35:
                        # 浅色/中等背景（页面或形状填充）：无颜色定义的文字强制设为深灰
                        if not dry_run:
                            _set_run_text_color_xml(run, 0x22, 0x22, 0x22)
                        fixes += 1
                    else:
...
def _replace_light_color_in_rPr(rPr_elem, dry_run: bool = False):
    """如果 rPr 元素中的 solidFill 是白色/浅色（亮度>0.7），替换为 #222222。"""
...
def _replace_light_color_in_rPr_v2(rPr_elem, dry_run: bool = False):
    ...
    处理两种情况：
    1. solidFill/srgbClr val="FFFFFF" 等浅色 → 改为 "222222"
    2. solidFill/schemeClr val="bg1|lt1|bg2|lt2" → 整个替换为 srgbClr val="222222"
...
def adapt(input_pptx: str, output_pptx: str,
          ...
          skip_white_black: bool = True) -> dict:
    """模式：
    ...
    skip_white_black: 跳过白/黑/灰等中性色替换（默认 True，避免误改正文文字）
    ...
    report = {
        ...
        "logo_skipped": 0,             # [P0-2] Logo 去重跳过数量
    ...
    for idx, slide in enumerate(pres.slides):
        ...
        if mode == "full":
            ...
            try:
                bg = assets["bg_cover"] if ptype in ("cover", "section", "end") else assets["bg_content"]
                if not dry_run:
        ...
        try:
            if not dry_run:
                added = add_logo(slide, ptype, assets["logo_main"],
                                 assets["logo_corner"], slide_w, slide_h)
                if not added:
                    report["logo_skipped"] += 1
        except Exception as e:
            report["warnings"].append(f"第 {idx+1} 页 Logo 添加失败：{e}")
...
def print_report(report: dict):
    ...
    print(f"Logo 去重跳过数量：{report.get('logo_skipped', 0)}")
...
def _run_html_pipeline(args):
    ...
    print(f"  品牌合规: 字体=TencentSans W7/W3 | 配色=品牌安全色 | 背景=模板背景 | Logo=已插入")
    print(f"{'='*60}")


def main():
    ap = argparse.ArgumentParser(
        description="腾讯云架构师长沙同盟 PPT 模板适配器 v8 — 自动识别输入格式",
    ...
    # PPTX 迁移模式参数
    ap.add_argument("--city", default="changsha", help="城市模板（默认 changsha）[仅 PPTX 模式]")
    ...
    ap.add_argument("--no-skip-white-black", action="store_false", dest="skip_white_black",
                    help="不跳过白/黑/灰色的替换 [仅 PPTX 模式]")
    ap.set_defaults(skip_white_black=True)
